...
/**
 展示UIView及其子类的占位图（本质是在这个view上添加一个自定义view）
...
- (void)cq_showPlaceholderViewWithFrame:(CGRect)frame type:(CQPlaceholderViewType)type reloadBlock:(void (^)())reloadBlock {
    // 延迟0.01秒是为了当使用的是自动布局时能够正确获取到frame
    // 详情：http://www.jianshu.com/p/e71bcc7a569e
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.cq_placeholderView = [[UIView alloc] initWithFrame:frame];
...
- (void)cq_showPlaceholderViewWithType:(CQPlaceholderViewType)type reloadBlock:(void (^)())reloadBlock {
    ...
    // 详情：http://www.jianshu.com/p/e71bcc7a569e
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self cq_showPlaceholderViewWithFrame:self.bounds type:type reloadBlock:reloadBlock];
    });
}
